-- ==========================================
-- RANKING SYSTEM OPTIMIZATION — Audit v1
-- FigusUY — 2026-05-05
-- ==========================================

-- 1. Update BUSINESS RANKING calculation
...
BEGIN
    ...
    -- Multiplier for views reduced from 2 to 0.5 as per audit recommendation.
    -- Each successful completion is worth 50 points in engagement.
    ...
    -- E. Activity (BASED ON REAL USAGE)
    -- If there are completions in the last 30 days, activity is high.
    SELECT count(*) INTO completion_count FROM exchange_completions 
    ...
    ELSE
        ...
        IF days_inactive <= 7 THEN s_activity := 80;
        ELSIF days_inactive <= 14 THEN s_activity := 50;
        ELSIF days_inactive <= 30 THEN s_activity := 25;
    ...
    RETURN jsonb_build_object(
        ...
        'badges', to_jsonb(biz_badges)
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 2. Update USER RANKING with Proactive Trust baseline
...
BEGIN
    ...
    -- B. Trust Score (PROACTIVE MODEL: BASELINE 50)
    SELECT count(*) INTO report_count FROM reports
    ...
    s_trust := 50; -- Baseline
    s_trust := s_trust + (confirmed_trades * 10); -- Gain trust by doing things
    s_trust := s_trust - (report_count * 25); -- Heavy penalty
    s_trust := s_trust - (block_count * 30);
    IF p.is_verified THEN s_trust := s_trust + 20; END IF;
    IF p.rating IS NOT NULL THEN s_trust := s_trust + (p.rating * 2); END IF;
    ...
    ELSIF days_inactive <= 7 THEN s_activity := 60;
    ELSIF days_inactive <= 14 THEN s_activity := 30;
    ELSE s_activity := 10; END IF;

    -- D. Match Quality Score (based on completion rate)
    ...
    ELSE
        s_quality := LEAST(chat_count * 10, 50);
    END IF;
    ...
    IF confirmed_trades >= 10 THEN user_badges := array_append(user_badges, 'veterano'); END IF;
