...
export default function AdminInfluencerCampaigns() {
  const { campaigns, affiliates, fetchCampaigns, fetchInfluencers, createCampaign, updateCampaign, duplicateCampaign, loading } = useInfluencerStore()
  ...
  useEffect(() => { fetchCampaigns(); fetchInfluencers() }, [])
  ...
  const activeInfluencers = affiliates.filter(a => a.status === 'activo')
