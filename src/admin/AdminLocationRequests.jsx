  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
            ...
            <div key={req.id} style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.75rem' }}>
                  {req.status === 'pending' && <span style={badge('#f59e0b', '#fffbeb')}>Pendiente</span>}
              ...
              {req.status === 'rejected' && req.rejection_reason && (
                <div style={{ marginTop: '1rem', padding: '0.75rem', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '0.5rem', fontSize: '0.8125rem', color: '#b91c1c' }}>
                  <strong>Razon del rechazo:</strong> {req.rejection_reason}
                </div>
              )}
            </div>
          ))}
      ...
      <ConfirmDialog
        ...
        message="El local se creara automaticamente y el acceso comercial quedara habilitado para este usuario."
        confirmText="Aprobar"
        cancelText="Cancelar"
        variant="info"
