    ...
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.875rem', fontWeight: 900, color: '#020617', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <span className="material-symbols-outlined" style={{ color: '#ef4444', fontSize: '2rem' }}>block</span>
            Gestión de Bloqueos
          </h1>
          <p style={{ fontSize: '0.9375rem', color: '#64748b', marginTop: '0.25rem' }}>
            Bloqueos activos, historial y desbloqueos con auditoría.
          </p>
        </div>
        <button onClick={() => setShowBlockModal(true)} style={{ padding: '0.625rem 1.25rem', borderRadius: '0.5rem', background: '#ef4444', color: 'white', border: 'none', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(14rem, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
        <div style={{ ...card, borderLeft: '4px solid #ef4444' }}>
          <p style={{ fontSize: '2rem', fontWeight: 900, color: '#0f172a' }}>{userBlocks.filter(b => b.is_active).length}</p>
        ...
        <div style={{ ...card, borderLeft: '4px solid #f59e0b' }}>
          <p style={{ fontSize: '2rem', fontWeight: 900, color: '#0f172a' }}>{userBlocks.filter(b => b.is_active && b.block_type === 'temporary').length}</p>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        {[{ key: 'active', label: 'Activos' }, { key: 'inactive', label: 'Historial' }, { key: 'all', label: 'Todos' }].map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} style={{
            ...
            background: filter === f.key ? '#ef4444' : '#fff', color: filter === f.key ? 'white' : '#64748b',
      ...
      <div style={card}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>No hay bloqueos en esta categoría.</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {filtered.map(block => (
              <div key={block.id} style={{ padding: '1rem', background: block.is_active ? '#fef2f2' : '#f8fafc', borderRadius: '0.75rem', border: `1px solid ${block.is_active ? '#fecaca' : '#f1f5f9'}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div>
                    <p style={{ fontWeight: 800, color: '#0f172a', margin: 0 }}>{block.user?.name || 'Usuario'}</p>
                    <p style={{ fontSize: '0.75rem', color: '#64748b', margin: '0.25rem 0' }}>{block.user?.email}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <span style={badge('#f1f5f9', '#64748b')}>{block.block_type}</span>
                  </div>
                </div>
                <p style={{ fontSize: '0.8125rem', color: '#475569', margin: '0.75rem 0 0.5rem' }}>
                ...
                {block.is_active && (
                  <button onClick={() => setUnblockId(block.id)} style={{ marginTop: '0.75rem', padding: '0.375rem 0.75rem', borderRadius: '0.5rem', background: '#ecfdf5', color: '#10b981', border: '1px solid #a7f3d0', fontSize: '0.8125rem', fontWeight: 700, cursor: 'pointer' }}>
                    Desbloquear
                  </button>
                )}
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div style={{ ...card, width: '100%', maxWidth: '28rem' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', marginBottom: '0.375rem', textTransform: 'uppercase' }}>Buscar Usuario</label>
              <input type="text" placeholder="Nombre o email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                style={{ width: '100%', padding: '0.625rem', borderRadius: '0.5rem', border: '1px solid #e2e8f0', marginBottom: '0.5rem' }} />
              {searchTerm && matchedUsers.length > 0 && (
                <div style={{ border: '1px solid #e2e8f0', borderRadius: '0.5rem', maxHeight: '8rem', overflowY: 'auto' }}>
                  {matchedUsers.map(u => (
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', marginBottom: '0.375rem', textTransform: 'uppercase' }}>Tipo</label>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                {['permanent', 'temporary'].map(t => (
                  <button key={t} type="button" onClick={() => setBlockForm({ ...blockForm, blockType: t })} style={{
                    ...
                    background: blockForm.blockType === t ? '#fef2f2' : 'white', color: blockForm.blockType === t ? '#ef4444' : '#64748b'
            ...
            {blockForm.blockType === 'temporary' && (
              <div style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', marginBottom: '0.375rem', textTransform: 'uppercase' }}>Expira</label>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 700, color: '#64748b', marginBottom: '0.375rem', textTransform: 'uppercase' }}>Motivo</label>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              <button onClick={() => setShowBlockModal(false)} style={{ background: 'none', border: 'none', color: '#64748b', fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div style={{ ...card, width: '100%', maxWidth: '28rem' }}>
            ...
            <textarea placeholder="Motivo del desbloqueo..." value={unblockReason} onChange={e => setUnblockReason(e.target.value)}
              style={{ width: '100%', minHeight: '5rem', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid #e2e8f0', fontFamily: 'inherit', marginBottom: '1.5rem' }} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              <button onClick={() => setUnblockId(null)} style={{ background: 'none', border: 'none', color: '#64748b', fontWeight: 600, cursor: 'pointer' }}>Cancelar</button>
