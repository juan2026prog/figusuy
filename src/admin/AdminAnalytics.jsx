...
export default function AdminAnalytics() {
  ...
  const kpis = [
    { label: 'DAU', value: d.dau, icon: 'bolt', color: 'var(--color-primary)', bg: '#fff7ed' },
    ...
    { label: 'Total Usuarios', value: d.total_users, icon: 'group', color: "#f5f5f5", bg: "var(--admin-panel2)" },
    ...
    { label: 'Exchanges OK', value: d.exchanges_completed, icon: 'verified', color: '#22c55e', bg: '#ecfdf5' },
    { label: 'Pend. confirm.', value: d.pending_confirmations, icon: 'hourglass_top', color: '#f59e0b', bg: '#fffbeb' },
    { label: 'Disputados', value: d.disputed_exchanges, icon: 'gavel', color: '#ef4444', bg: '#fef2f2' },
    { label: 'Cierre chat->exchange', value: d.chat_to_exchange_rate != null ? `${Number(d.chat_to_exchange_rate).toFixed(1)}%` : '0%', icon: 'conversion_path', color: '#38bdf8', bg: '#ecfeff' },
    { label: 'Locales Activos', value: d.active_locations, icon: 'storefront', color: 'var(--color-primary)', bg: '#fff7ed' },
    ...
    { label: 'Suscripciones Usr', value: d.active_subscriptions, icon: 'card_membership', color: 'var(--color-primary)', bg: '#fff7ed' },
    { label: 'Suscripciones Neg.', value: d.active_business_subs, icon: 'store', color: "#f5f5f5", bg: "var(--admin-panel2)" },
  ]
  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '1.875rem', fontWeight: 900, color: "#f5f5f5", letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <span className="material-symbols-outlined" style={{ color: 'var(--color-primary)', fontSize: '2rem' }}>analytics</span>
      ...
      {loading && !analyticsData ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: "var(--admin-muted)" }}>Cargando analytics...</div>
      ) : (
        <>
          {/* KPI Grid */}
          ...
          <div style={{ ...card, marginBottom: '2rem' }}>
            <h3 style={{ fontWeight: 800, marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span className="material-symbols-outlined" style={{ color: 'var(--color-primary)' }}>show_chart</span>
              Actividad Diaria (Últimos 14 días)
            </h3>
            {dailyActivity.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '2rem', color: "var(--admin-muted)" }}>Sin datos de actividad diaria disponibles.</div>
            ) : (
              <>
                  ...
                  {dailyActivity.map((day, i) => {
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
                        <span style={{ fontSize: '0.5625rem', fontWeight: 700, color: "var(--admin-muted2)" }}>{day.active_users || 0}</span>
                        <div style={{ width: '100%', background: 'var(--color-primary)', height: `${h}%`, borderRadius: '0.25rem 0.25rem 0 0', minHeight: '4px', transition: 'height 0.3s' }} />
                      </div>
                    )
                  })}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.5rem', color: "var(--admin-muted)", fontSize: '0.5625rem', fontWeight: 600 }}>
                  {dailyActivity.map((day, i) => (
            ...
            <div style={card}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {dailyActivity.slice(-7).map((day, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem', background: "var(--admin-panel2)", borderRadius: '0.375rem', fontSize: '0.8125rem' }}>
                    <span style={{ color: "var(--admin-muted)" }}>{day.date ? new Date(day.date).toLocaleDateString('es-UY', { weekday: 'short', day: '2-digit' }) : ''}</span>
