...
export default function AdminSettings() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.25rem' }}>⚙️ Configuración General</h1>
      <p style={{ fontSize: '0.875rem', color: '#64748b', marginBottom: '1.5rem' }}>Controlá toda la plataforma desde acá</p>
      ...
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} style={{ ...card, marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {items.map(s => (
              <div key={s.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid #f1f5f9' }}>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: '#0f172a' }}>{s.key.replace(/_/g, ' ')}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  {editKey === s.key ? (
                    <>
                      ...
                      <button onClick={() => setEditKey(null)} style={{ padding: '0.375rem 0.625rem', borderRadius: '0.375rem', background: '#f1f5f9', color: '#64748b', border: 'none', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer' }}>✕</button>
                    </>
                  ) : (
                    <>
                      <code style={{ fontSize: '0.75rem', background: '#f1f5f9', padding: '0.25rem 0.5rem', borderRadius: '0.25rem', color: '#475569' }}>
                        {JSON.stringify(s.value).replace(/"/g, '')}
                      </code>
                      <button onClick={() => { setEditKey(s.key); setEditVal(typeof s.value === 'string' ? s.value : JSON.stringify(s.value)) }} style={{ padding: '0.25rem 0.5rem', borderRadius: '0.375rem', background: '#eff6ff', color: '#3b82f6', border: 'none', fontSize: '0.6875rem', fontWeight: 600, cursor: 'pointer' }}>✏️</button>
                    </>
                  )}
                </div>
              </div>
